...
function JuegoCollection(juegoCollection){
    this.games = juegoCollection;
    ...
    this.toHTML = function(){
        ...
        $.each(this.games, function(i, v) {
            var juego = v;
            html = html.concat('<br>');
            html = html.concat('<strong> CreationDate: </strong>' + juego.creationdate + '<br>');
            html = html.concat('<strong> Description: </strong>' + juego.description + '<br>');
            html = html.concat('<strong> Id: </strong>'  + juego.id + '<br>');
            html = html.concat('<strong> Url: </strong>' + juego.url + '<br>');
            html = html.concat('<strong> UserList: </strong>' + juego.userlist + '<br>');
            html = html.concat('<br>');
        });
        ...
        console.log(prev);
        if (prev.length == 1) {
            html = html.concat(' <a onClick="getjuegos(\'' + prev[0].href + '\');" style="cursor: pointer; cursor: hand;">[Anterior]</a> ');

        }
        var next = this.getLink('next');
        if (next.length == 1) {
            html = html.concat(' <a onClick="getjuegos(\'' + next[0].href + '\');" style="cursor: pointer; cursor: hand;">[Siguiente]</a> ');
        }
        return html;
    }
}
...
function getjuegos(url){
    ...
    $.ajax({
        ...
        var juegoCollection = new JuegoCollection(response);
        var linkHeader = jqxhr.getResponseHeader('Link');
        juegoCollection.buildLinks(linkHeader);
        var html = juegoCollection.toHTML();
        $("#result").html(html);
...
function getJuegos(){
    ...
    $.ajax({
        ...
        var juegos = data.games;
        $.each(juegos, function(i, j) {
            var juego = j;

            $('<br><strong> FechaDeCreacion: </strong> ' + juego.creationdate + '<br>').appendTo($('#result'));
            $('<strong> Que coño es esto: </strong> ' + juego.description + '<br>').appendTo($('#result'));
            $('<strong> El Id: </strong> ' + juego.id + '<br>').appendTo($('#result'));
            $('<strong> Su Url es: </strong> ' + juego.url + '<br>').appendTo($('#result'));
            $('<strong> Listado de Usuarios: </strong> ' + juego.userlist + '<br>').appendTo($('#result'));
        });
...
function deleteGame(id) {
    ...
    $.ajax({
        ...
        .done(function(data, status, jqxhr) {
                $('<div class="alert alert-success"> <strong>Ok!</strong> Game deleted</div>').appendTo($("#result"));
...
$("#button_post").click(function(e){
    ...
    if($('#id').val() == "" || $('#url').val()=="") {
        ...
        console.log ("hola2");
        $('<div class="alert alert-info">Debes rellenar los campos de todos los Jugadores</div>').appendTo($("#result"));
    }
    else{
...
function createGame(Game) {
    ...
    var manuela = JSON.stringify(Game);
